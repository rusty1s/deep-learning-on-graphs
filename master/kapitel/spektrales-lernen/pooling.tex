...
Pooling-Schichten eines Netzes, üblicherweise über Max-Pooling realisiert, werden dabei für gewöhnlich direkt nach einer Faltungsschicht benutzt und sorgen dafür die Ausgabe der Faltung zu filtern \bzw{} zu vereinfachen~\cite{Nielsen}.
...
So muss dieser vorallem als ein mehrstufiges Clustering fungieren, bei dem jede Stufe eines Graphen den Blick auf den Graphen bei einer unterschiedlichen \enquote{Auflösung} zeigt und insbesondere die zu Grunde liegende Geometrie des Graphen erhält~\cite{Defferrard}.
...
Falls $\gls{v}_i$ keinen unmarkierten Nachbarsknoten besitzt, wird $\gls{v}_i$ alleine als Superknoten $\gls{v}^* \coloneqq \left\{ v \right\} \in \gls{V}_{l+1}$ deklariert und markiert.
...
\end{equation*}
der die Kantengewichte relativ zum Knotengrad gewichtet~\cite{Defferrard, graclus}.
Der Normalized-Cut sorgt dafür, dass Kanten als wichtiger gezählt werden, wenn ihre entsprechenden Knoten einen geringen Grad besitzen und damit folglich als unwahrscheinlicher gelten, mit einem anderen Superknoten zu verschmelzen.
...
In der Praxis zeigt sich jedoch, dass Graclus nur sehr wenige Superknoten mit einem einzigen Knoten generiert~\cite{Defferrard}.
Weiterhin ist anzumerken, dass der Algorithmus bei mehrmaliger Anwendung auf dem gleichen Graphen aufgrund seiner zufälligen Iteration auf den Knoten zwei verschiedene, vergröberte Graphen generiert.
