...
\end{equation}
vom Grad $K$ mit Koeffizienten $\left(c_0, \ldots, c_K\right) \in \gls{R}$ approximiert werden~\cite{Hammond, Defferrard}.
...
\end{equation}
wobei die Werte $\left(c_0, \ldots, c_K\right) \in \gls{R}$ nun die Koeffizienten der Tschebyschow-Polynome $\gls{T}_k\left(\gls{Lambdatilde}\right) \in \gls{R}^{N \times N}$ bilden~\cite{Defferrard}.
