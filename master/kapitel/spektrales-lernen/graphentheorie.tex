...
Die Eigenwerte und Eigenvektoren von \gls{Lboth} helfen uns beim Verständnis der linearen Transformation einer Funktion \ve{f} (mehrfach) angewendet auf \gls{Lboth}.
Wir können dafür \ve{f} als Linearkombination der Eigenbasis $\ve{f} = \sum_{n=1}^N a_n \gls{eiv}_n$, $a_n \in \gls{R}$ schreiben und erhalten
\begin{equation*}
