...
% \centering
\begin{algorithmic}
  \REQUIRE{} Initiale Knotenverfärbung $\ve{h}^{\left(1\right)} \in \gls{R}^N$
  \ENSURE{} Finale Knotenverfärbung $\ve{h}^{\left(T\right)} \in \gls{R}^N$ nach $T$ Durchläufen
\STATE $z = 2$
\WHILE{$(z < 10)$}
\STATE $x = x + z$
\FOR{$(1 \leq a \leq z-1)$}
...
\ENDWHILE
\end{algorithmic}
\caption[Ein Algorithmus]{Algorithmus}
\label{algo_1}
\end{algorithm}
