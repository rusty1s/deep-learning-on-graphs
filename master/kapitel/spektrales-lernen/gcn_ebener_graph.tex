...
Kanten, die vorher zwar in einer gleichen Partition lagen, sich jedoch \evtl{} stark in ihrer Ausrichtung unterschieden, sind nun \enquote{eindeutig} differenzierbar über ihre abweichenden Auswertungen von ${\left\{\gls{basis}_p^K\right\}}_{p=0}^{P-1}$ \bzw{} ihrer Anteile von $\left(c_0, \ldots, c_{P-1}\right)$ an $\gls{spline}\left(\alpha\right)$ entsprechend ihrer unterschiedlichen Winkel.
...
Mit der expliziten Forderung gleichmäßig verteiler Knoten im Intervallbereich ergibt sich dann der Knotenvektor $\ve{\tau} \coloneqq \left(\alpha_0, \ldots, \alpha_{P+K}\right)$ mit $\alpha_p \coloneqq 2\pi p/P$.
...
Für die aktuelle Partition $p$ im linken Summanden muss daher keine Veränderung vorgenommen werden und kann analog zu~\eqref{eq:bspline_basis_schritt} übernommen werden.
Lediglich im rechten Summanden muss darauf geachtet werden, \enquote{nicht über die Grenzen hinaus zu laufen}.
Für den Grenzfall $p = P-1$ wird daher im zweiten Summanden die Basisfunktion $\gls{basis}_0^k\left(\alpha\right)$ addressiert.
...
\end{equation*}
mit den freien Parametern $\left(c_0, \ldots, c_P\right) \in \gls{R}$, wobei $\gls{basis}_p^K$ elementweise auf die Matrix \gls{Arad} angewendet wird.
...
Für $K=0$ ist die Faltung über den B-Splines äquivalent zu der Faltung über den Partitionen des Graphen aus~\eqref{eq:partitionierung_faltung} aufgrund der Rechteckfunktionen $\gls{basis}_p^0\left(\alpha\right)$.
