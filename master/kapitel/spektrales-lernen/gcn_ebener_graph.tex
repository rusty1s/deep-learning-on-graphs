...
\\\\
Obwohl die Faltung auf den Partitionen eines Graphen insbesondere durch die Äquivalenz zur klassischen Faltung auf regulären Gittern vielversprechend erscheint, gehen dennoch Informationen über die Ausrichtung von Kanten verloren.
So werden Kanten mit Ausrichtungen im Intervall $\left(2\pi\left(p-1\right)/P, 2\pi p/P\right]$ stets gleich behandelt, auch wenn sich diese \evtl{} extrem unterscheiden.
Eine Lösung zu diesem Problem ist sicherlich, $P$ insoweit zu erhöhen, dass das Intervall der Innenwinkel der Partitionen entsprechend klein wird.
