...
Eine Basisfunktion $\gls{basis}_i^K \colon \left[t_0, t_{m+1}\right] \to \left[0, 1\right]$ ist dabei mit Hilfe eines \emph{Knotenvektors} $\ve{\tau} = {\left[t_0, \ldots, t_{m+K+1}\right]}^{\top} \in \gls{R+}^{m+K+2}$, $t_i \leq t_{i+1}$, rekursiv über $K$ definiert mit der Initialisierung für $k=0$
\begin{equation}
