...
Sei \gls{G} ein Graph im zweidimensionalen euklidschen Raum, eindeutig definiert über $\gls{Adist} \in \gls{R+}^{N \times N}$ und $\gls{Arad} \in {\left[0, 2\pi\right]}^{N \times N}$.
...
\end{equation}
Damit beschreiben die Matrizen ${\left\{\gls{A}_p\right\}}_{p=1}^P$ disjunkte Partionen der Kanten des Graphen \gls{G} abhängig von ihren Ausrichtungen im Raum mit $\gls{Adist} = \sum_{p=1}^P \gls{A}_p$.
$\gls{A}_p$ ist insbesondere nicht zwingend symmetrisch, da ${\left(\gls{Arad}\right)}_{ij} \neq {\left(\gls{Arad}\right)}_{ji}$ für alle $\gls{v}_i, \gls{v}_j \in \gls{V}$ mit $\gls{v}_i \gls{adj} \gls{v}_j$.
Abbildung~\ref{fig:partitionierung} veranschaulicht den Prozess der Partitionierung mit $P = 8$ Bereichen bei gleichmäßigen Innenwinkeln $\pi/4$.
