...
\begin{tabular}{lccccccccc}
  \toprule
  \textbf{\gls{SLIC}} & $\hat{x}$ & $\hat{y}$ & $\mathrm{ecc}$ & $\mathrm{dia}$ & $\mathrm{ext}$ & $\gls{mu}^{\prime}_{20}$ & $\gls{lambda}_2$ & $\mathrm{axis}_1$ & $\mathrm{axis}_2$\\
  \textbf{Quickshift} & $\hat{x}$ & $\mathrm{ecc}$ & $\mathrm{dia}$ & $\mathrm{ext}$ & $\gls{mu}_{03}$ & $\gls{mu}_{21}$ & $\gls{mu}_{30}$ & $\gls{eta}_{03}$ & $\mathrm{ori}$\\
  \bottomrule
\end{tabular}
...
\end{table}
Wohingegen sich die Merkmalsselektion für \gls{SLIC} eher für Formmerkmale entscheidet, die aus den Momenten gewonnen werden können, so genießen für Quickshift die reinen translationsinvarianten Momente \gls{mu} größeres Interesse.
