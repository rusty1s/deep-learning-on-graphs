...
Shaping the output of our linear function into the form we want - in this case, a probability distribution over the classes.
\emph{Softmax}~\cite{tensorflow}
\begin{equation*}
  \mathrm{softmax}\left(\ve{x}\right) = \frac{\exp\left(\ve{x}\right)}{\sum_{n=1}^N \exp\left(\ve{x}_n\right)}
\end{equation*}
Exponentierten der Inputs und dann normalisieren
output ist immer größer 0
Normalisieren, so dass sie sich zu 1 aufaddieren (was einer Wahrscheinlichkeitsverteilung entspricht)

\emph{Cross-Entropie}~\cite{tensorflow}
\begin{equation*}
  H_{\ve{y}^{\prime}}\left(\ve{y}\right) \coloneqq - \sum_{n=1}^N \ve{y}_n^{\prime} \log \left(\ve{y}_n\right)
\end{equation*}
Kostenfunktion ist immer positiv
Je näher es am resultat ist, umso kleiner werden die Kosten


Warum ist das besser als Quadratic Cost

Loss Function
Accuracy

\emph{Genauigkeit} (\engl{} \emph{Accuracy})
\begin{equation*}
  \mathrm{accuracy}\left(\right)
\end{equation*}
...
So gut wie alle implementierten mathematischen Operationen in \texttt{tensorflow} besitzen eine Gradientenimplementierung, sodass die Kosten der Ausgabe eines Berechnungsgraph über das Backpropagationverfahren minimiert werden können.
Dies erlaubt insbesondere den Bau der implementierten Faltungsoperatoren auf Graphen mit den reinen Operationen, die \texttt{tensorflow} bereitstellt.
...
Sie enthält die Implementierungen zu den \gls{SLIC} und Quickshift Algorithmen.
Sie enthält ebenso eine Implementierung der Graphgenerierung aus einer Superpixelrepräsentation, die sich aber in Tests als ineffizient herausstellte und daher in dieser Arbeit nicht verwendet wird (\vgl{} Kapitel~\ref{laufzeitanalyse}).

Für die Merkmalsextraktion, die Graphgenerierung sowie die Implementierung der Graphvergröberungen für die Poolingoperationen wurden die Bibliotheken \texttt{numpy} und \texttt{scipy} verwendet~\cite{numpy, scipy}.
Beide Bibliotheken gehören aufgrund ihrer effizienten Implementierung in C zu der Standardwahl \bzgl{} wissenschaflicher Berechnungen in Python.
...
Dem Benutzer steht es dabei frei, die vorverarbeiten Daten in einem eigenen Datensatz vor Trainingsbeginn zu speichern oder alle notwendigen Vorverarbeitungsschritte der Bilder zur Laufzeit während des Trainings für das aktuell zu lernende Bild zu vollziehen.
...
Für größere Datensätze wie \gls{Pascal} ist dies aber nicht zu empfehlen, da der Aufwand der Vorverarbeitung durch die größeren Bilder, gerade aufgrund der steigenden Laufzeiten der Superpixelalgorithmen, zu einer erhöhten Lerndauer führt.
