...
Auffällig ist für \gls{SLIC} die Wahl des Merkmals \enquote{$\mathrm{box}$}, da dieses bereits durch \enquote{$\mathrm{box}_y$} und \enquote{$\mathrm{box}_x$} implizit gegeben ist.
...
$H\left(\mathcal{X}\right)$ ist aufgrund der Intervalleinschränkung der jeweiligen Ausgaben \ve{y} von $\ve{x} \in \mathcal{X}$ mit Hilfe der Softmax Regression stets positiv und wird umso kleiner, je ähnlicher sich $\ve{y}^{\prime}$ und dessen Ground-Truth $\ve{y}$ werden~\cite{Nielsen}.
