...
Die Vorverarbeitung während des Trainings wurde dabei mit Hilfe mehrerer Threads implementiert, sodass die Geschwindigkeit der Lernprozedur möglichst nicht gestört wird.
Es zeigt sich, dass für kleine Datensätze wie \gls{MNIST} oder \gls{Cifar}-10 die Vorverarbeitung der Daten während des Trainings so gut wie keinen Einfluss auf die Lerngeschwindigkeit mit sich zieht.
Für größere Datensätze wie \gls{Pascal} ist dies aber nicht zu empfehlen, da der Aufwand der Vorverarbeitung durch die größeren Bilder, gerade aufgrund der Laufzeiten der Superpixelalgorithmen, zu einer erhöhten Lerndauer führt.
Eine gezielte Laufzeitanalyse der beiden Vorverarbeitungsmethoden finden sich für die einzelnen Datensätze in Kapitel~\ref{laufzeitanalyse}.
