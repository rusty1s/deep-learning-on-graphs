...
Ein Graph $\gls{G} = \left(\gls{V}, \gls{E}, \gls{w}\right)$ kann weiterhin eindeutig über dessen (in der Regel dünnbesetzte) \emph{Adjazenzmatrix} $\gls{A} \in \gls{R+}^{N \times N}$ mit $\gls{A}_{ij} \coloneqq \gls{w}\left(\gls{v}_i, \gls{v}_j\right)$ definiert werden.
...
Wir nennen \gls{G} \emph{zusammenhängend}, wenn \gls{G} genau eine Zusammenhangskomponente besitzt, \dhe{} zu jedem Knoten $\gls{v}_i$ existiert mindestens ein Weg zu jedem anderen Knoten $\gls{v}_j \in \gls{V}$.
...
Eine Merkmalsfunktion $f$ kann ebenso als Vektor $\ve{f} \in \gls{R}^N$ mit $ \ve{f}_n \coloneqq f\left(\gls{v}_n\right)$ geschrieben werden.
Bildet $f$ weiterhin auf eine Menge von Merkmalen $f \colon \gls{V} \to \gls{R}^M$, $M \in \gls{N}$, ab, so kann diese analog über eine \emph{Merkmalsmatrix} $\ma{F} \in \gls{R}^{N \times M}$ mit $\ma{F}_{nm} \coloneqq {\left(f\left(\gls{v}_n\right)\right)}_m$ definiert werden.
