...
Der räumliche Faltungsoperator von~\cite{patchy} hat das Ziel, den Graphen in einen dreidimensionalen Tensor zu konvertieren, sodass die klassische Faltungsoperation \gls{conv2d} auf diesen angewendet werden kann.
Der Prozess zur \emph{Vektorisierung} eines Graphen besteht dabei aus drei Schritten: (1) Bestimme eine Knotenauswahl des Graphen mit einer definierten Länge, (2) finde für jeden ausgewählten Knoten eine Nachbarschaftsgruppierung und (3) normalisiere die jeweiligen Nachbarschaften zu einer festlegten Größe und bestimme auf diesen eine Ordnung, sodass Graphen mit ähnlichen Nachbarschaftsstrukturen eine äquivalente Ordnung erzeugen~\cite{patchy}.
Im Folgenden werden die drei Schritte näher erläutert.

\paragraph{Knotenauswahl}
\label{knotenauswahl}

Die Knotenauswahl beschreibt den Prozess
